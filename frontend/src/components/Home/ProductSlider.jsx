    ...
    <>
      <section className="py-16">
        <div className="flex items-center justify-between mb-8">
          <h2 className="tet-3xl font-bold text-foreground">{title}</h2>
            ...
            hover:glow-on-hover animate-smooth group"
              >
                ...
                <div className="relative overflow-hidden rounded-lg mb-4">
                  <img
                    scr={product.images[0].url}
                    alt={product.name}
                    className="w-full h-48 object-contain group-hover:scale-110 transition-transform duration-300"
                  />

                  {/* BADGES */}
                  <div className="absolute top-3 left-3 flex-col space-y-2">
                    {new Date() - new Date(product.createdAt) <
                      30 * 24 * 60 * 1000 && (
                      <span
                        className="px-2 py-1 bg-primary text-primary-foreground text-xs
                      font-semibold reounded"
                      >
                        NEW
                      </span>
                    )}
                ...
                <div>
                  {/* PRODUCT TITLE */}
                  <h3
                    className="text-lg font-semiboldtext-foreground mb-2 group-hover:text-primary 
                  transition-colors"
                  >
                    {product.name}
                  </h3>
